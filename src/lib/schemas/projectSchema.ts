...
export const estadoOptions = ['idea', 'en_desarrollo', 'finalizado', 'presentado', 'archivado', 'cancelado'] as const;
...
export const stringToArray = (val: string | string[] | undefined | null): string[] => {
  if (typeof val === 'string' && val.trim() !== '') {
    return val.split(',').map(item => item.trim()).filter(item => item.length > 0);
  }
  if (Array.isArray(val)) {
...
const AttachedFileSchema = z.object({
  ...
  url: z.string().url("Debe ser una URL válida.").min(1, "La URL del archivo es requerida."),
...
export const projectSchema = z.object({
  ...
  descripcionGeneral: z.string().min(10, "La descripción general debe tener al menos 10 caracteres."),
  ...
  anoProyecto: z.coerce.number().min(1900, "El año debe ser válido.").max(new Date().getFullYear() + 5, "El año no puede ser tan futuro."),
  ...
  archivoPrincipalURL: z.string().url("Debe ser una URL válida.").or(z.literal('')).optional().nullable(),
...
export const convertFormDataForFirestore = (data: ProjectFormData): Record<string, any> => {
  const firestoreData: { [key: string]: any } = { ...data };
  
  const arrayIdKeys: (keyof Pick<ProjectFormData, 'idsTemas' | 'idsAutores' | 'idsTutoresPersonas' | 'idsColaboradores' | 'idsOrganizacionesTutoria' | 'palabrasClave'>)[] = [
    'idsTemas', 'idsAutores', 'idsTutoresPersonas', 'idsColaboradores', 'idsOrganizacionesTutoria', 'palabrasClave'
  ];
  ...
  const optionalStringKeys: (keyof Pick<ProjectFormData, 'resumenEjecutivo' | 'archivoPrincipalURL' | 'nombreArchivoPrincipal'>)[] = [
  ...
  if (firestoreData.archivosAdjuntos && Array.isArray(firestoreData.archivosAdjuntos)) {
      firestoreData.archivosAdjuntos = firestoreData.archivosAdjuntos.map((adjunto: any) => {
        const newAdjunto = { ...adjunto };
        newAdjunto.tipo = adjunto.tipo || null;
        newAdjunto.descripcion = adjunto.descripcion || null;
        return newAdjunto;
      });
      if(firestoreData.archivosAdjuntos.length === 0) firestoreData.archivosAdjuntos = null;
  } else {
    firestoreData.archivosAdjuntos = null;
  }


  firestoreData.fechaInicio = data.fechaInicio ? Timestamp.fromDate(data.fechaInicio) : null;
  firestoreData.fechaFinalizacionEstimada = data.fechaFinalizacionEstimada ? Timestamp.fromDate(data.fechaFinalizacionEstimada) : null;
  firestoreData.fechaFinalizacionReal = data.fechaFinalizacionReal ? Timestamp.fromDate(data.fechaFinalizacionReal) : null;
  firestoreData.fechaPresentacion = data.fechaPresentacion ? Timestamp.fromDate(data.fechaPresentacion) : null;
  
  return firestoreData;
};

export const convertFirestoreDataToForm = (projectData: Partial<Proyecto>): ProjectFormData => {
  const convertTimestampToDate = (timestamp: any): Date | null => timestamp && typeof timestamp.toDate === 'function' ? timestamp.toDate() : null;
  
  const formValues: { [key: string]: any } = { ...projectData };

  formValues.fechaInicio = convertTimestampToDate(projectData.fechaInicio);
  formValues.fechaFinalizacionEstimada = convertTimestampToDate(projectData.fechaFinalizacionEstimada);
  formValues.fechaFinalizacionReal = convertTimestampToDate(projectData.fechaFinalizacionReal);
  formValues.fechaPresentacion = convertTimestampToDate(projectData.fechaPresentacion);
  ...
  formValues.idsTutoresPersonas = arrayFieldToArray(projectData.idsTutoresPersonas);
  formValues.idsOrganizacionesTutoria = arrayFieldToArray(projectData.idsOrganizacionesTutoria); 
