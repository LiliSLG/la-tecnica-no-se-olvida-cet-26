...
class PersonaTemaService {
  async addTemaToPersona(personaId: string, temaId: string): Promise<ServiceResult<PersonaTema | null>> {
    try {
      if (!personaId || !temaId) {
        return createError({
  ...
  async removeTemaFromPersona(personaId: string, temaId: string): Promise<ServiceResult<boolean>> {
    try {
      if (!personaId || !temaId) {
        return createError({
          name: 'ValidationError',
  ...
  async getTemasByPersonaId(personaId: string): Promise<ServiceResult<PersonaTema[] | null>> {
    try {
      if (!personaId) {
        return createError({
          name: 'ValidationError',
  ...
  async getPersonasByTemaId(temaId: string): Promise<ServiceResult<PersonaTema[] | null>> {
    try {
      if (!temaId) {
        return createError({
          name: 'ValidationError',
