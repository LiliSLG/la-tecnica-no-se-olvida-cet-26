...
const formSchema = z.object({
  ...
  ano_proyecto: z.coerce.number()
    .int("El año debe ser un número entero.")
    .min(1980, { message: "El año no parece válido." })
    .max(new Date().getFullYear() + 1, { message: "El año no puede ser tan a futuro." }),
  estado_actual: z.enum(["idea", "en_desarrollo", "finalizado", "presentado", "archivado", "cancelado"], {
...
export function ProyectoForm({ initialData }: ProyectoFormProps) {
  ...
  async function onSubmit(values: z.infer<typeof formSchema>) {
    if (!user) {
      toast({ title: "Error de autenticación", description: "Debes iniciar sesión para realizar esta acción.", variant: "destructive" });
      return;
    }

    try {
      if (initialData) { // Estamos en modo EDICIÓN
        const dataToUpdate: UpdateProyecto = {
          ...values,
          actualizado_por_uid: user.id,
        };
      ...
      toast({ title: "Error en el servidor", description: "No se pudo guardar el proyecto. Intenta de nuevo.", variant: "destructive" });
    }
  }
        ...
        <FormField
          ...
          render={({ field }) => (
            <FormItem>
              ...
              <FormControl>
                <Input placeholder="Ej: Invernadero Automatizado con Arduino" {...field} />
              </FormControl>
              <FormMessage />
            </FormItem>
          )}
