...
export function ProyectosClientPage({ allProyectos }: ProyectosClientPageProps) {
  const router = useRouter();
  const { toast } = useToast();
  const { user } = useAuth();
  const [proyectos, setProyectos] = useState(allProyectos);
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      toast({ title: "Proyecto eliminado", description: "El proyecto se ha marcado como eliminado." });
      router.refresh();
    ...
    { key: 'estado_actual', label: 'Estado', sortable: true },
    {
      ...
      render: (proyecto: Proyecto) => (
        <div className="flex items-center gap-2">
          <Button variant="ghost" size="icon" onClick={() => router.push(`/admin/proyectos/${proyecto.id}`)}><Eye className="h-4 w-4" /></Button>
          <Button variant="ghost" size="icon" onClick={() => router.push(`/admin/proyectos/${proyecto.id}/edit`)}><Pencil className="h-4 w-4" /></Button>
          <Button variant="ghost" size="icon" onClick={() => setProyectoToDelete(proyecto)}><Trash2 className="h-4 w-4 text-red-600" /></Button>
        </div>
      )
    }
  ];
  ...
  return (
    <div className="p-6">
      ...
      <AlertDialog open={!!proyectoToDelete} onOpenChange={() => setProyectoToDelete(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>¿Está seguro?</AlertDialogTitle>
            <AlertDialogDescription>Esta acción marcará el proyecto como eliminado. Podrá restaurarlo más tarde.</AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
