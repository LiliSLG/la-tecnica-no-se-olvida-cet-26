    ...
    if (user && !authLoading) {
      const fetchPersona = async () => {
        ...
        try {
          ...
          if (error || !data) {
            console.warn(
              "No se encontró fila de persona para UID:",
              user.id,
              "— tal vez aún no se creó."
            );
            setPersonaData(null);
          } else {
          ...
          console.error("Error al traer datos de persona:", err);
          setPersonaData(null);
        } finally {
    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-[calc(100vh-200px)] text-center p-6">
        ...
        <p className="text-muted-foreground mb-6">
          Debes iniciar sesión para acceder al panel de gestión.
        </p>
        <Button
          onClick={() => router.push("/login?redirect=/management-panel")}
        >
          Iniciar Sesión
        </Button>
      </div>
    ...
    <div className="space-y-12">
      ...
      <Card className="shadow-xl">
        <CardHeader>
          ...
          <CardDescription>
            Tu información y actividad en la plataforma.
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-6">
          <div className="flex flex-wrap items-center gap-4 p-4 border rounded-lg bg-card/50">
            ...
            <div className="space-y-1 flex-grow">
              ...
              {personaData ? (
                <>
                  <p className="text-muted-foreground flex items-center gap-2">
                    <Briefcase className="h-4 w-4" /> Categoría Principal:{" "}
                    {personaData.categoriaPrincipal || "No especificada"}
                  </p>
                  <p className="text-muted-foreground flex items-center gap-2">
                    Estado:{" "}
                    {personaData.activo ? (
                ...
                <p className="text-sm text-muted-foreground">
                  Información de perfil no encontrada. Se creará/sincronizará en
                  breve si es tu primer inicio de sesión.
                </p>
              )}
            </div>
            <Button
          ...
          <div>
            <h4 className="text-xl font-semibold text-primary/90 mb-3 flex items-center gap-2">
              <MicVocal className="h-5 w-5" /> Mis Entrevistas
            </h4>
            <div className="p-4 border rounded-lg bg-background text-center text-muted-foreground">
              <p>
                (Aquí se listarán las entrevistas que realizaste - Próximamente)
              </p>
            </div>
          </div>
        </CardContent>
